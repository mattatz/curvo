...
use nalgebra::{
    Const, DefaultAllocator, DimName, DimNameAdd, DimNameDiff, DimNameSub, OMatrix, OPoint,
    OVector, RealField, Rotation3, UnitVector3, Vector3, U1,
};
    ...
    DefaultAllocator: Allocator<D>,
{
    ...
    ///     Point2::new(0.0, 1.0),
    /// ];
    /// let mut curve = NurbsCurve2D::interpolate(&points, 3).unwrap();
    /// curve.invert();
    /// let (start, end) = curve.knots_domain();
